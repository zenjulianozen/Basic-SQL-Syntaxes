--- Execute a failing query (i.e. one which gives an error, yes, really) to retrieve all 
...
--- Execute a working query using a sub-select to retrieve all employees records whose 
...
SELECT * 
FROM (SELECT EMP_ID, F_NAME, L_NAME, DEP_ID FROM EMPLOYEES) AS EMPLOYEE_HIDE
